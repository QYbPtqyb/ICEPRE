...
from evaluation.nemesysUtils import protocols
from evaluation.nemesysUtils.tsharkConnector import TsharkConnector, TsharkOneshot
...
class ParsedMessage(object):
    ...
    def _parseJSON(self, dissectjson: List[Tuple[str, any]]):
        ...
        * self._fieldsflat
        ...
        tcpkey = 'tcp'
        layersvalue = ParsedMessage._getElementByName(ParsedMessage._getElementByName(dissectjson, sourcekey), layerskey)
        if layersvalue:
            frameraw = ParsedMessage._getElementByName(layersvalue, framekey + ParsedMessage.RK)

            # add
            # get the length of tcp payload
            tcppayload_raw = ParsedMessage._getElementByName(ParsedMessage._getElementByName(layersvalue, tcpkey), 'tcp.payload'+ParsedMessage.RK)
            self.tcp_payload_len = tcppayload_raw[2]
            #

            if len(frameraw) >= 1 and isinstance(frameraw[0], str):
                self.framebytes = frameraw[0]
            protocolsvalue = ParsedMessage._getElementByName(ParsedMessage._getElementByName(layersvalue, framekey), protocolskey)
            if len(protocolsvalue) == 1 and isinstance(protocolsvalue[0], str):
                ...
                # Changed
                absLayNum = self.protocols.index('tcp') + 1
                try:
                    ...
                    if rawProtocols[:len(self.protocols)] == self.protocols:
                        ...
                        IPython.embed()
                        raise e
                f_dissectsub = ParsedMessage._getElementByName(layersvalue, self.protocolname)
                # fix:
                if isinstance(f_dissectsub[0], list):
                    temp = list()
                    for item_sub in f_dissectsub:
                        temp.extend(item_sub)
                    # print(isinstance(temp[0], tuple))
                    self._dissectfull = temp
                else:
                    self._dissectfull = f_dissectsub

                # add missing layers in protocols list
                pKeys = [a for a, b in layersvalue]
                # debug
                # print('-----frame.protocols:')
                ...
                self.protocolbytes = ParsedMessage._getElementByName(layersvalue, self.protocolname + ParsedMessage.RK)  # tshark 2.2.6
                ...
                if absLayNum < len(self.protocols):
                    for embedded in self.protocols[absLayNum+1 : ]:
                        ...
                        if isinstance(dissectsub, list) and len(dissectsub) != 0:
                            if isinstance(dissectsub[0], list):
                                temp = list()
                                for item_sub in dissectsub:
                                    temp.extend(item_sub)
                                # print(isinstance(temp[0], tuple))
                                self._dissectfull += temp
                            else:
                ...
                self._fieldsflat = ParsedMessage.walkSubTree(self._dissectfull, allSubFields=True)

                # print('-----self._fieldsflat:')
                # print(self._fieldsflat)
                # print("-----self._dissectfull:")
                # print(self._dissectfull)
                # print('-----branch nodes:')
                ...
                #     import difflib
                #     import evaluation.nemesysUtils.bcolors as bcolors
    ...
    def generate_groundtruth(self):
        """
        Generate correct field boundaries from a message

        Returns
        -------
        e.g. [0, 1, 0, 1, 0, 0, 0, 1, 0]
        """
        true_boundary = [0] * (self.tcp_payload_len - 1)
        # The offset in the TCP payload
        offset = -1
        # The offset in the entire message
        flag_offset = 0
        for k, v in self._fieldsflat:
            if offset == -1:
                flag_offset = v[1]
            if flag_offset == v[1]:
                offset = offset + v[2]
                if offset >= (self.tcp_payload_len - 1):
                    break
                true_boundary[offset] = 1
                flag_offset += v[2]
            elif v[1] > flag_offset:
                offset = offset + (v[1] - flag_offset)
                if offset >= (self.tcp_payload_len - 1):
                    break
                true_boundary[offset] = 1
    ...
    def walkSubTree(root: List[Tuple[str, any]], allSubFields=False) -> List[Tuple[str, str]]:
        ...
        for fieldkey, subnode in root:
            ...
            # branch node, ignore textual descriptions
                ...
                # changed:
                if len(subfields) != 0 and len(subfields[-1]) != 0 and subfields[-1][0] == fieldkey:
                    subfields.pop()
                ...
                if allSubFields and fieldkey not in CONSTANTS_CLASS.IGNORE_FIELDS \
                    ...
                    # changed
                    # subfields.extend(ParsedMessage.walkSubTree(nodevalue, fieldkey in CONSTANTS_CLASS.RECORD_STRUCTURE))
                    subfields.extend(ParsedMessage.walkSubTree(nodevalue, allSubFields=True))
