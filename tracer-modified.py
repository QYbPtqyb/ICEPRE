...
class Tracer(ExplorationTechnique):
    ...
    def step_state(self, simgr, state, **kwargs):
        ...
        if not self._follow_unsat:
            ...
            if len(sat_succs) == 1:
                ...
                # added by myself
                for unconstrained_state in succs_dict["unconstrained"]:
                    ...
                    succs_dict["unconstrained"] = []
                    succs_dict[None] = [unconstrained_state]
                    break
    ...
    def _update_state_tracking(self, state: "SimState"):
        ...
        if sync == "entry":
            ...
            raise TracerDesyncError(
                f"Oops! angr did not follow the trace {state.addr}", deviating_addr=state.addr, deviating_trace_idx=idx + 1
            )
